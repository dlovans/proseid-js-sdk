...
// src/frameProtocol.js
var FRAME_PROTOCOL = "proseid-embed-v1";
...
function frameMessage(channel, type, payload = {}) {
...
// src/FrameClient.js
var KEY_RE = /^proseid_pk_[a-f0-9]{32,64}$/;
var CALLBACK_NAMES = /* @__PURE__ */ new Set(["ready", "change", "validation", "submit", "signing", "complete", "receipt", "language", "error"]);
...
var ProseIDFrame = class {
  constructor(target, options = {}) {
    ...
    const path = options.testMode ? "/embed/v1/test" : `/embed/v1/flows/${encodeURIComponent(coordinate.publisher)}/${encodeURIComponent(coordinate.slug)}`;
  ...
  onMessage(event) {
    ...
    if (message.type === "resize") {
      const height = Math.max(180, Math.min(1e5, Math.ceil(Number(message.height) || 0)));
...
function mount(target, options) {
  return new ProseIDFrame(target, options);
}
function mountTest(target, options) {
  return new ProseIDFrame(target, { ...options, testMode: true });
}
